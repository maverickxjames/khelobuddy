...
<!--  </a>-->
  ...
  if($conflict){
      ?>
    <a href="" class="btn btn-success btn-sm  rounded-60 w-100 fw-bold my-1"  data-bs-toggle="offcanvas" data-bs-target="#uss">
  <span>upload screenshot for conflict</span>
  </a>
   <div class="offcanvas offcanvas-bottom" style="height:50vh" tabindex="-1" id="uss" aria-labelledby="offcanvasBottomLabel">
   <div class="offcanvas-header">
     <h5 class="offcanvas-title" id="offcanvasBottomLabel">Upload Screenshot</h5>
    <button type="button" class="btn-close" data-bs-dismiss="offcanvas" aria-label="Close"></button>
   </div>
   <div class="offcanvas-body">
   <form method="post" enctype="multipart/form-data" action="<?=base_url('user/uss/'.$match->id)?>" class="showloadingform">
 <div class="mb-3">
 ...
 <p class="text-danger small"><b>Note:</b> if your opponent has Exit or AutoExit submit screenshot or contact admin</p> <button type="submit" class="btn btn-sm btn-success w-100">Upload Screenshot</button>
 </form>
   </div>
 </div>
 
 <a href="" class="btn btn-success btn-sm  rounded-60 w-100 fw-bold my-1"  data-bs-toggle="offcanvas" data-bs-target="#upl">
  <span>sumbit lost for conflict</span>
  </a>
   <div class="offcanvas offcanvas-bottom" style="height:50vh" tabindex="-1" id="upl" aria-labelledby="offcanvasBottomLabel">
   <div class="offcanvas-header">
     <h5 class="offcanvas-title" id="offcanvasBottomLabel">Submit Lost</h5>
    <button type="button" class="btn-close" data-bs-dismiss="offcanvas" aria-label="Close"></button>
   </div>
   <div class="offcanvas-body">
   <form method="post" enctype="multipart/form-data" action="<?=base_url('user/uplost/'.$match->id)?>" class="showloadingform">
 <div class="mb-3">
   <label for="formFileSm" class="form-label">Submit Lost</label>
   <input class="form-control form-control-sm" id="formFileSm" name="screenshot" type="file" required>
 </div>
 <p class="text-danger small"><b>Note:</b>if you Exit or AutoExit submit lost</p> <button type="submit" class="btn btn-sm btn-success w-100">Submit Lost</button>
 </form>
   </div>
 </div>
  <?php
...
if($this->db->get('system')->row()->instagram){
  ...
  <span> <i class="bi bi-instagram"></i> Connect On Instagram <b>FREE LOOT!!!</b></span>
  </a>
    <?php
}
?>
...
if($hostcancel){
    ?>
<span class="badge small bg-secondary small"> Cancellation Request Submitted</span>
    <?php
}
?>
</div>
...
<div class="d-flex">
<?php
if($system->ssallgame==1 or $match->game==32){
if($match->winner!=$user->id && $match->looser!=$user->id){
     ?>
 <a href="" class="btn btn-success btn-sm  rounded-60 w-100 fw-bold my-1"  data-bs-toggle="offcanvas" data-bs-target="#winscreenshot"><i class="bi bi-hand-thumbs-up"></i> I Won The Match</a>
 ...
 <p class="text-danger small"><b>Note:</b> <?=$this->db->get('messages')->row()->on_win_pop_up?></p> <button type="submit" class="btn btn-sm btn-success w-100">Submit Result As Winner</button>
 </form>
   </div>
 </div>
...
if($match->winner!=$user->id && $match->looser!=$user->id){
    ?>
<a href="" class="btn btn-danger btn-sm  rounded-60 w-100 fw-bold my-1"  data-bs-toggle="offcanvas" data-bs-target="#losescreenshot"><i class="bi bi-hand-thumbs-down"></i> I Lost The Match</a>
...
if($match->winner!=$user->id && $match->looser!=$user->id){
    ?>
<a href="" class="btn btn-primary btn-sm  rounded-60 w-100 fw-bold my-1"  data-bs-toggle="offcanvas" data-bs-target="#update"></i>Update Match</a>

  <div class="offcanvas offcanvas-bottom" style="height:50vh" tabindex="-1" id="update" aria-labelledby="offcanvasBottomLabel">
  <div class="offcanvas-header">
    <h5 class="offcanvas-title" id="offcanvasBottomLabel">Update Match</h5>
    <button type="button" class="btn-close" data-bs-dismiss="offcanvas" aria-label="Close"></button>
  </div>
  <div class="offcanvas-body">
      <?php
if($system->aup==1){
    ?>
  <form method="post" enctype="multipart/form-data" action="<?=base_url('user/updatematch/'.$match->id)?>" class="showloadingform">
    <?php
}
else{
?>
    <form method="post" enctype="multipart/form-data" action="<?=base_url('user/mupdatematch/'.$match->id)?>" class="showloadingform">
    <?php
}
?>
<p class="text-danger small"><b>Note:</b> <?=$this->db->get('messages')->row()->on_lose_pop_up?></p>
<button type="submit" class="btn btn-sm btn-danger w-100">Update Match</button>
</form>
  </div>
</div>
    <?php
