...
<!--  <input type="text" class="form-control" placeholder="UPI ID" id="upi" aria-label="" aria-describedby="basic-addon1">-->
<!--</div>-->
...
<option>Bank</option>
    ...
    <div class="input-group mb-3 nonbank" style="display:none">
  <span class="input-group-text" id="basic-addon1"><i class="bi bi-bank"></i></span>
  <input type="text" class="form-control" placeholder="UPI ID" id="upi" aria-label="" aria-describedby="basic-addon1">
</div>

  <div class="input-group mb-3 bank">
  <span class="input-group-text" id="basic-addon1"><i class="bi bi-person"></i></span>
  <?php
  if($b){
  ?>
  <input type="text" class="form-control" placeholder="Account Holder Full Name" id="name_in_bank" aria-label="" aria-describedby="basic-addon1" value="<?=$b[0]->name?>" >
  <?php
  }
  else{
      ?>
      <input type="text" class="form-control" placeholder="Account Holder Full Name" id="name_in_bank" aria-label="" aria-describedby="basic-addon1">
  <?php
  }
  ?>
</div>
...
<div class="input-group mb-3 bank">
  ...
  if($b){
  ?>
  <input type="text" class="form-control" placeholder="Bank IFSC Code" id="bank_ifsc_code" aria-label="" aria-describedby="basic-addon1" value="<?=$b[0]->bank_ifsc_code?>" >
  <?php
  }
  else{
      ?>
...
<div class="input-group mb-3 bank">
  ...
  if($b){
  ?>
  <input type="number" class="form-control" placeholder="Bank Account Number" id="bank_ac_no" aria-label="" aria-describedby="basic-addon1" value="<?=$b[0]->bank_ac_no?>" >
  <?php
  }
  else{
      ?>
...
<div class="small text-success">Minimum Withdraw : ₹ <?=$this->db->get('system')->row()->minimum_withdraw?></div>
    <?php
    if(!($this->db->get('system')->row()->withdrawallow)){
    ?>
    <button href="#" class="btn w-100 btn-dark my-2" id="nomoney_btn" >Withdraw will be available at 8:00 am</button>
    <?php
    }
    else if(!$first){
    ?>
    ...
    else if(($this->db->get('system')->row()->witht)==1 and (time()-$first[0]->created_at)<3600){
    ?>
    <button href="#" class="btn w-100 btn-dark my-2" id="w1hrmoney_btn" >Withdraw Availaible in <?=gmdate('H:i:s', 3600-(time()-($first[0]->created_at)))?> </button>
    <?php
    }
    else if(($this->db->get('system')->row()->witht)==2 and (time()-$first[0]->created_at)<7200){
    ?>
    <button href="#" class="btn w-100 btn-dark my-2" id="w2hrmoney_btn" >Withdraw Availaible in <?=gmdate('H:i:s', 7200-(time()-($first[0]->created_at)))?> </button>
    <?php
    }
    else{
    ?>
